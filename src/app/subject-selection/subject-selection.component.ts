...
export class SubjectSelectionComponent{
  ...
  stopAllSounds() {
    ...
    window.speechSynthesis.cancel(); // إيقاف النطق الآلي
  }
  ...
  getSubjects(): string[] {
    if ((this.selectedSection === 'عربي' && this.selectedGrade === 'KG1')||(this.selectedSection === 'لغات' && this.selectedGrade === 'KG1')) {
      return this.schoolStructure[this.selectedSection].subjects.filter(subject => subject !== 'English Crystal');
    } else {
